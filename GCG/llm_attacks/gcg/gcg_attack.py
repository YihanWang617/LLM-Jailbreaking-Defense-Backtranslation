...
from llm_attacks.base.custom import ModelWorkerCustom
            ...
            for j, cand in enumerate(control_cands):
                ...
                for i in progress:
                    for k, worker in enumerate(self.workers):
                        # Only the new `ModelWorkerCustom` is supported right now
                        # due to a newly added memory optimization.
                        assert isinstance(worker, ModelWorkerCustom)
                        worker(self.prompts[k][i], "logits", worker.model, cand, return_ids=True)
                    logits, ids = zip(*[worker.results.get() for worker in self.workers])
                    loss[j*batch_size:(j+1)*batch_size] += sum([
                        target_weight *
                        self.prompts[k][i].target_loss(
                            logit, id,
                            # Do slicing within wthe worker to save some memory,
                            # since logits can be very large
                            sliced=True
                        ).mean(dim=-1).to(main_device)
