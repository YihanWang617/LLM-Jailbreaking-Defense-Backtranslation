...
from transformers import (AutoModelForCausalLM, AutoTokenizer, GPT2LMHeadModel,
                          ...
                          LlamaForCausalLM)
from .custom import ModelWorkerCustom
...
class AttackPrompt(object):
    ...
    def _update_ids(self):
        ...
        if self.conv_template.name == 'oasst_pythia':
            return self._update_ids_oasst_pythia(prompt, encoding, toks)

        self.conv_template.messages = []
        self.conv_template.append_message(self.conv_template.roles[0], None)

        if self.conv_template.name in ['llama-2', 'llama-2-new']:
            offset = 0
        elif self.conv_template.name == 'vicuna_v1.1':
            offset = 1
        else:
            raise NotImplementedError(self.conv_template.name)
        ...
        self._goal_slice = slice(self._user_role_slice.stop,
                                 max(self._user_role_slice.stop, len(toks)-offset))

        separator = ' ' if self.goal else ''
        self.conv_template.update_last_message(f"{self.goal}{separator}{self.control}")
        toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
        self._control_slice = slice(self._goal_slice.stop, len(toks)-offset)
        ...
        toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
        self._target_slice = slice(self._assistant_role_slice.stop, len(toks)-2+offset)
        self._loss_slice = slice(self._assistant_role_slice.stop-1, len(toks)-3+offset)

        self.input_ids = torch.tensor(toks[:self._target_slice.stop], device='cpu')
        self.conv_template.messages = []

    def _update_ids_oasst_pythia(self, prompt, encoding, toks):
        ...
        self._user_role_slice = slice(
            ...
            encoding.char_to_token(prompt.find(self.conv_template.roles[0])
                                    + len(self.conv_template.roles[0]) + 1)
        )
        ...
        self._assistant_role_slice = slice(
            ...
            encoding.char_to_token(prompt.find(self.conv_template.roles[1])
                                    + len(self.conv_template.roles[1]) + 1)
        )
    ...
    def target_loss(self, logits, ids, sliced=False):
        crit = nn.CrossEntropyLoss(reduction='none')
        if not sliced:
            loss_slice = slice(self._target_slice.start-1, self._target_slice.stop-1)
            logits = logits[:,loss_slice,:]
            ids = ids[:,self._target_slice]
        ...
        for step, control in enumerate(controls):
            for (mode, goals, targets) in zip(*[('Train', 'Test'), (self.goals, self.test_goals), (self.targets, self.test_targets)]):
                if control != prev_control and len(goals) > 0:
                    ...
                    if verbose:
                        print(f"{mode} Step {step+1}/{len(controls)} | "
...
def get_workers(params, eval=False):
    ...
    for conv in raw_conv_templates:
        if conv.name == 'zero_shot':
            ...
            conv.sep = '\n'
        elif conv.name in ['llama-2', 'llama-2-new']:
